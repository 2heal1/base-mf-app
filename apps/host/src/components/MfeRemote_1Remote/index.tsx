import { mount } from "mfe-remote-1/bootstrap";
import {
  useLocation,
  useNavigate,
  useNavigationType,
...
const mfeRemote_1Basename = `/${mfeRemote_1RoutingPrefix}`;

export default function MfeRemote_1Remote(props: DivProps) {
  ...
  useEffect(() => {
    const mfeRemote_1NavEventHandler = (event: Event) => {
      const { pathname, search, navigationType } = (event as NavigationEvent)
        .detail;
      const newPathname = `${mfeRemote_1Basename}${pathname}${search}`;
    ...
    window.addEventListener(
      `[${mfeRemote_1RoutingPrefix}] navigated`,
      mfeRemote_1NavEventHandler,
    );
  ...
  useEffect(() => {
    if (location.pathname.startsWith(mfeRemote_1Basename)) {
      window.dispatchEvent(
        new NavigationEvent(`[${shellRoutingPrefix}] navigated`, {
          detail: {
            pathname: location.pathname.replace(mfeRemote_1Basename, ""),
  ...
  useEffect(() => {
    ...
    unmountRef.current = mount({
      ...
      shellRoutingPrefix,
      appRoutingPrefix: mfeRemote_1RoutingPrefix,
    }) as UnmountFn;
  ...
  return <div ref={wrapperRef} id="mfe-remote-1-mfe" {...props} />;
}
